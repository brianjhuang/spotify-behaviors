...
def recommendation(pre_vars):
    ...
    dropDown = st.selectbox('What song would you like to pick?', (tuple(songs)))
    song = self.get_song_id(dropDown)
    ...
    userDown = dropDown = st.selectbox('What user would you like to pick?', userTuple)
    
    userDict = {'One': userOneFeatures
           'Two': userTwoFeatures,
           'Three': userThreeFeatures}
    
    model = songRecommender(data = userDict[userDown], predict = features)
